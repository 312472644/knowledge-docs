...
对象类型：Object。
这些数据类型可分为原始数据类型和对象类型。基本类型存储在栈(stack)中，而复杂数据类型则存储在堆中。这两种数据类型的区别其实就是堆栈的区别。
...
```javascript
async/await其实是Generator 的语法糖，都是协程的一种实现模式。async/await 是有栈协程，有独立的运行栈，Generator是 无线协程，没有独立的运行栈。有栈协程从流程上的调度更容易控制，但它需要独立的内存空间，而无线协程则相对于更节省资源。它能实现的效果都能用then链来实现，它是为优化then链而开发出来的。
...
Watcher类作用就是收集相关依赖。
每个组件都会有一个watcher实例，当实例化这个对象时，会触发构造函数中的this.get()方法，get方法中的pushTarget方法就是把Dep.target 赋值为当前的watcher。在这个过程中会触发数据对象的getter，每个数据对象都会持有一个dep，在触发getter时候会调用dep.depend() 方法和dep.addSub()。将当前的watcher订阅到Dep的订阅列表中。当监听数据发生变化时,会触发Dep中的notify方法，然后通知对应的订阅者，实现对应的操作。
```
...
getters：类似于computed属性，通过对state进行处理，返回处理后state数据。
modules：模块划分。
...
```javascript
一个基于浏览器原生 ES Modules 的开发服务器。利用浏览器去解析模块，在服务器端按需编译返回，完全跳过了打包这个概念，服务器随起随用。Vite 要求项目完全由 ES Module 模块组成，cjs不能直接在vite上使用(浏览器运行的是cjs)，所以打包上还是使用的rollup。
vite实现主要是通过启动一个 koa 服务器拦截浏览器请求ES Module的请求，对不同的文件类型做出不同的处理，最终以 ES Modules 格式返回给客户端。大致流程如下
1、首先判断请求是否有缓存，有则直接读取缓存，否则就进行下一步处理。
